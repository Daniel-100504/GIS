...
if ($action === 'list') {
    requireMenro();
    $rows = $pdo->query("SELECT id, name, barangay, stated_ha, source, coordinates FROM protected_areas ORDER BY name")->fetchAll(PDO::FETCH_ASSOC);
    ...
    $features = array_map(function ($row) {
        return [
            ...
            'properties' => [
                ...
                'statedHa' => $row['stated_ha'] !== null ? (float) $row['stated_ha'] : null,
                'source'   => $row['source'],
            ],
...
if ($action === 'update') {
    ...
    $stmt = $pdo->prepare("UPDATE protected_areas SET " . implode(', ', $fields) . " WHERE id = :id");
    $stmt->execute($params);
